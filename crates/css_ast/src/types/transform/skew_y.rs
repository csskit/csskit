...
impl<'a> Peek<'a> for SkewY {
	fn peek(p: &Parser<'a>, c: Cursor) -> bool {
		<T![Function]>::peek(p, c) && p.eq_ignore_ascii_case(c, "skewy")
	}
}

impl<'a> Parse<'a> for SkewY {
	fn parse(p: &mut Parser<'a>) -> ParserResult<Self> {
		let skew = p.parse::<T![Function]>()?;
		let c: Cursor = skew.into();
		if !p.eq_ignore_ascii_case(c, "skewy") {
...
mod tests {
	...
	#[test]
	fn size_test() {
		assert_eq!(std::mem::size_of::<SkewY>(), 44);
	}

	#[test]
	fn test_writes() {
		assert_parse!(SkewY, "skewY(1deg");
		assert_parse!(SkewY, "skewY(0)");
	}

	#[test]
	fn test_errors() {
		assert_parse_error!(SkewY, "skewY()");
		assert_parse_error!(SkewY, "skewY(foo)");
	}
}
