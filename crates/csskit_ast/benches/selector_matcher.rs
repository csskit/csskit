...
fn selector_matching(c: &mut Criterion) {
				...
				b.iter_with_large_drop(|| {
					let allocator = Bump::default();
					{
						...
						if let (Some(stylesheet), Some(sheet)) = (css_result.output, query_result.output) {
							// Extract first node rule selector for benchmarking
							for rule in &sheet.rules {
								if let Rule::NodeRule(node_rule) = rule {
									let matcher = SelectorMatcher::new(
										&node_rule.selector,
										&query_file.source_text,
										&css_file.source_text,
									);
									let _matches: Vec<_> = black_box(matcher.run(&stylesheet).collect());
									break;
								}
							}
						}
